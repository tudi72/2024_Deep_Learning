...
#############################################################################################################################################
        ...
        self.attn = nn.MultiheadAttention(embedding_dim, 4, 0.1, batch_first=True)
#############################################################################################################################################
...
#############################################################################################################################################
        # EMBEDDINGS        [256,23, 128]
        # CAPTION INDICES   [ 256, 22]
        # HIDDEN STATE      NONE
        x = encoded_image
        ...
        output = rearrange(attn_weights[:, 0, 1:], "b (h w) -> b h w", h=th)
        
 ############################################################################################################################################

        output, hidden_state = self.rnn(input=output, hx=hidden_state)

        logits = self.to_logits(output)
        # input tensor [256,2994], wrong shape expected 3, received 2-dim
